...
insight-map-loader run       # the service: ingest -> bridge -> alignment -> shared-frame
                 # MPT1 to `out`. Bridging is AUTOMATIC (see below); `insight-map-loader
...
Layout: `insight-map-loader-core/` is the UI-free library (wire format, ingest, transforms,
bridge, fleet, aggregator -- 13 unit tests) plus the `insight-map-loader` CLI binary.
`insight-map-loader-gui/` (egui) is the same pipeline with a window on it: fleet cards with
...
service while it is open -- run either the GUI or `insight-map-loader run`, not both (they
...
It listens on udp/5181 — the insight-map-loader service's `out` — because 5180 on this
